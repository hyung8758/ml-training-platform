...
def validate_environment() -> None:
    ...
    placeholder = [
        name for name in REQUIRED_VARIABLES if "<" in os.environ.get(name, "")
    ]
    if missing:
        raise RuntimeError(f"필수 환경변수가 없습니다: {', '.join(missing)}")
    if placeholder:
        raise RuntimeError(
            f"placeholder를 실제 값으로 변경하세요: {', '.join(placeholder)}"
        )
...
def check_connection() -> str:
    ...
    try:
        ...
        raise RuntimeError(
            "ClearML SDK가 없습니다. `pip install -e .`를 실행하세요."
        ) from error
...
def main() -> int:
    ...
    # CLI 경계에서는 SDK와 네트워크 오류를 한글 안내로 통일해 보여준다.
