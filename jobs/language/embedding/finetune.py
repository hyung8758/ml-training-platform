# Embedding Fine-tuning Job의 설정, ClearML Task와 저장 경로를 준비한다.
# Framework별 command와 실행 책임은 선택한 Backend runner에 위임한다.
...
JOB_TYPE = "language.embedding.finetune"


def validate_configuration(
    config: dict[str, Any], capabilities: Mapping[str, Any]
) -> None:
    """Embedding Fine-tuning 공통 설정과 Backend 호환성을 검증한다."""
    require_fields(
        config,
        (
            "experiment.project",
...
def load_configuration(
    ...
    """Embedding Fine-tuning YAML을 읽고 플랫폼 호환 정책까지 검증한다."""
...
def main() -> None:
    ...
    task, config = initialize_task(
        initial_config, config_path=args.config, extra_tags=("embedding", "finetune")
    )
